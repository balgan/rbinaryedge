#' Query dataleak database - domain
#'
#' Query dataleak database for domain
#'
#' @import httr
#' @importFrom jsonlite fromJSON
#' @importFrom data.table as.data.table
#'
#' @param domain target domain
#' @param auth_token authentication token for the API
#' @export


domaindataleakQuery <- function(domain, auth_token) {

  content_query <- GET(paste0("https://api.binaryedge.io/v2/query/dataleaks/organization/",domain),add_headers("X-Key" = auth_token))
  raise <- content(content_query, as="text", encoding = 'UTF-8')

  fromJSON(raise, flatten = TRUE) %>%
    as.data.table() %>%
    unnest() %>%
    replace(.=="NULL", NA)



}

